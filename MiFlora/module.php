...
class MiFlora extends IPSModule
{
    ...
    public function Update():void
    {
        ...
        $mifloraHubs   = json_decode($mifloraConfig, false);
        ...
        if (@is_array($sensorArray)) {
            ...
            if ($this->ReadPropertyBoolean('CreateReadableTextEnabled')) {
                ...
                if (isset($readableText['fertilizePlease'])) {
                    /** @noinspection PhpUndefinedVariableInspection */
                    $readableText_fertilizePlease =
                        'Folgende Pflanzen benötigen heute noch Dünger: ' . implode(',', $readableText['fertilizePlease']);
                } else {
    ...
    private function logThis($message, $logLevel, $logToDisc = false):void
    {
        ...
        switch ($logLevel) {
            case self::LOGLEVEL_DEBUG:
                IPS_LogMessage(__CLASS__ . ' [DBG]', $message);
                break;
        ...
        foreach ($sensorList as $sensor) {
            ...
            if ($formSensor !== false) {
                ...
                if (isset($formSensor['mediaID']) && $formSensor['mediaID'] > 0) {
                    ...
                    $this->CreateVariable(
                        'Standort', 3, "<center><img style='width:100%;' src='/user/MiFlora/" . $uuid . ".jpg'></center>", $uuid . '_place', $catID,
                        '~HTMLBox'
                    );
                }
            }
